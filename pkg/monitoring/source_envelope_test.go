...
func TestEnvelopeSource(t *testing.T) {
	var blockHeight int64 = 123456789
	...
	latestConfigDetailsRes := []byte(fmt.Sprintf("{\"config_count\":1,\"block_number\":%d,\"config_digest\":[0,2,40,124,212,36,212,11,158,183,88,56,225,63,84,249,32,189,3,145,59,110,99,100,131,77,141,26,136,52,166,231]}", blockHeight))
	...
	chainReader.On("TxsEvents",
		...
		mock.MatchedBy(func(query []string) bool {
			return query[0] == fmt.Sprintf("tx.height=%d", blockHeight) &&
				query[1] == fmt.Sprintf(`wasm-set_config.contract_address='%s'`, feedConfig.ContractAddressBech32)
		}),
	...
	require.Equal(t, envelope.BlockNumber, uint64(blockHeight))
	...
	// Prepare for another execution of fetch. The config should be served from cache.
	chainReader.On("ContractStore",
		mock.Anything, // context
		feedConfig.ContractAddress,
		[]byte(`"latest_config_details"`),
