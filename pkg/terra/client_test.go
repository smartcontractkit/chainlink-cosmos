...
func TestTerraClient(t *testing.T) {
	...
	sn, err := tc.SequenceNumber(accounts[0].Address)
	require.NoError(t, err)
	resp, err := tc.SignAndBroadcast(msg.NewMsgSend(accounts[0].Address, accounts[1].Address, msg.NewCoins(msg.NewInt64Coin("uluna", 1))),
		sn+1, tc.GasPrice(), accounts[0].PrivateKey)
	require.NoError(t, err)
	...
	sn, err = tc.SequenceNumber(accounts[0].Address)
	require.NoError(t, err)
	_, err = tc.SignAndBroadcast(rawMsg, sn+1, tc.GasPrice(), accounts[0].PrivateKey)
	require.NoError(t, err)
	time.Sleep(1 * time.Second)
