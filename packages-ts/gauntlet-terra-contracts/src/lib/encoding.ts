...
export const serializeOffchainConfig = async (input: OffchainConfig, randomSecret?: string): Promise<Buffer> => {
  ...
  const sharedSecretEncryptions = await generateSecretEncryptions(configPublicKeys, randomSecret)
