...
import { endWasmd, CMD_FLAGS, maybeInitWasmd, NODE_URL, TIMEOUT, deployAC, deployLink, toAddr, DEFAULT_GAS_PRICE, deployOCR2 } from '../utils'
...
describe('OCR2 Execution', () => {
  ...
  const verifyDeployment = async () => {
    ...
    // expect(await OCR2.latestTransmissionDetails()).toEqual({
    //     latest_config_digest: new Array(32).fill(0),
    //     epoch: 0,
    //     round: 0,
    //     latest_answer: '0',
    //     latest_timestamp: 0,
    ...
    // expect(await OCR2.latestRoundData()).toEqual({
    //     round_id: 0,
    //     answer: '0',
    ...
    // expect(await OCR2.owedPayment({ transmitter: deployerAddr })).toEqual('0')
    ...
    // expect(await OCR2.oracleObservationCount({ transmitter: deployerAddr })).toEqual(0)
  ...
  it('Withdraw Remaining Funds To Chosen Receipient', async () => {
