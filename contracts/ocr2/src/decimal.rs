//! This is based on ab2eeb4 from cosmwasm-std (from the 1.0.0 branch).
//! We needed the multiplication implementation for two decimals that is not present on 0.16.
//! Our fork also makes the Decimal constructor public so that raw u128 values can be converted to
//! decimals. https://github.com/CosmWasm/cosmwasm/issues/1155
...
use cosmwasm_std::{Fraction, OverflowError, OverflowOperation, StdError, Uint128, Uint256};

/// A fixed-point decimal value with 18 fractional digits, i.e. Decimal(1_000_000_000_000_000_000) == 1.0
///
/// The greatest possible value that can be represented is 340282366920938463463.374607431768211455 (which is (2^128 - 1) / 10^18)
#[derive(Copy, Clone, Default, Debug, PartialEq, Eq, PartialOrd, Ord, JsonSchema)]
pub struct Decimal(#[schemars(with = "String")] pub Uint128);
    ...
    pub fn from_atomics(
        ...
        Ok(match decimal_places.cmp(&(Self::DECIMAL_PLACES as u32)) {
            Ordering::Less => {
                ...
                Self(
                    atomics
                        .checked_mul(Uint128::from(factor))
            ...
            Ordering::Greater => {
                ...
                if let Ok(factor) = TEN
                    .checked_pow(digits)
                    .ok_or_else(|| OverflowError::new(OverflowOperation::Pow, TEN, digits))
                {
                    Self(atomics.checked_div(Uint128::from(factor)).unwrap()) // Safe because factor cannot be zero
                } else {
...
impl ops::Mul for Decimal {
    ...
    fn mul(self, other: Self) -> Self {
        ...
        let result_as_uint256 =
            self.numerator().full_mul(other.numerator()) / Uint256::from(Self::DECIMAL_FRACTIONAL); // from_uint128 is a const method and should be "free"
...
mod tests {
    ...
    fn uint128_decimal_multiply() {
        ...
        assert_eq!(left * right, Decimal::from_str("450").unwrap());
